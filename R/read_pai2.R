...
SPECTRUM_PEAK_KEY_MAP <- c(
  ...
  "7" = "PeakID",
  "11" = "SpectrumComment",
  "12" = "IsAbsolutelyRequiredFragmentForAnnotation",
  "13" = "FragmentationScore"
)

FORMULA_KEY_MAP <- c(
  "0" = "FormulaString",
  "1" = "Mass",
  "2" = "M1IsotopicAbundance",
  "3" = "M2IsotopicAbundance",
  "4" = "Cnum",
  "5" = "Nnum",
  "6" = "Hnum",
  "7" = "Onum",
...
ADDUCT_ION_KEY_MAP <- c(
  "0" = "AdductIonAccurateMass",
  "1" = "AdductIonXmer",
  "2" = "AdductIonName",
  "3" = "ChargeNumber",
  "4" = "IonMode",
...
MS_SCAN_MATCH_RESULT_KEY_MAP <- c(
  "0" = "Name",
  "1" = "InChIKey",
  "2" = "TotalScore",
  "3" = "SquaredWeightedDotProduct",
  "4" = "SquaredSimpleDotProduct",
  "5" = "SquaredReverseDotProduct",
  "6" = "MatchedPeaksCount",
  "7" = "MatchedPeaksPercentage",
  "8" = "EssentialFragmentMatchedScore",
  "9" = "RtSimilarity",
  "10" = "RiSimilarity",
  "11" = "CcsSimilarity",
  "12" = "IsotopeSimilarity",
  "13" = "AcurateMassSimilarity",
  "14" = "LibraryID",
  "15" = "IsPrecursorMzMatch",
  "16" = "IsSpectrumMatch",
  "17" = "IsRtMatch",
  "18" = "IsCcsMatch",
  "19" = "IsLipidClassMatch",
  "20" = "IsLipidChainsMatch",
  "21" = "IsLipidPositionMatch",
  "22" = "IsOtherLipidMatch",
  "23" = "IsRiMatch",
  "24" = "LibraryIDWhenOrdered",
  "26" = "Source",
  "27" = "AnnotatorID",
  "28" = "SpectrumID",
  "29" = "AndromedaScore",
  ...
  "31" = "Priority",
  "32" = "PEPScore",
  "33" = "IsReferenceMatched",
  "34" = "IsAnnotationSuggested",
  "35" = "IsLipidDoubleBondPositionMatch",
  "36" = "CollisionEnergy"
)

MS_SCAN_MATCH_RESULT_CONTAINER_KEY_MAP <- c(
  "0" = "MatchResults",
  "1" = "MSRawID2MspBasedMatchResult",
  "2" = "TextDbBasedMatchResults"
)

ION_FEATURE_CHARACTER_KEY_MAP <- c(
  "0" = "AdductType",
  "1" = "AdductTypeByAmalgamationProgram",
  "2" = "Charge",
  "3" = "PeakLinks",
  "4" = "IsotopeWeightNumber",
  "5" = "IsotopeParentPeakID",
  "6" = "PeakGroupID",
  "7" = "IsLinked",
  "8" = "AdductParent"
)

LINKED_PEAK_FEATURE_KEY_MAP <- c(
  "0" = "LinkedPeakID",
  "1" = "Character"
)

CHROMATOGRAM_PEAK_SHAPE_KEY_MAP <- c(
  "0" = "EstimatedNoise",
  "1" = "SignalToNoise",
  "2" = "PeakPureValue",
  "3" = "ShapenessValue",
  "4" = "GaussianSimilarityValue",
  "5" = "IdealSlopeValue",
  "6" = "BasePeakValue",
  "7" = "SymmetryValue",
  "8" = "AmplitudeOrderValue",
  "9" = "AmplitudeScoreValue"
)

FEATURE_FILTER_STATUS_KEY_MAP <- c(
  "0" = "IsAbundanceFiltered",
  "1" = "IsRefMatchedFiltered",
  "2" = "IsSuggestedFiltered",
  "3" = "IsUnknownFiltered",
  "4" = "IsBlankFiltered",
  "5" = "IsMsmsContainedFiltered",
  "6" = "IsFragmentExistFiltered",
  "7" = "IsCommentFiltered"
)


key_map_to_slot_names <- function(key_map, length_out) {
  slots <- paste0("..slot_", seq_len(length_out) - 1L)
  idx <- as.integer(names(key_map)) + 1L
  valid <- idx >= 1L & idx <= length_out
  slots[idx[valid]] <- unname(key_map[valid])
  slots
}

name_slots_by_key_map <- function(x, key_map) {
  if (!is.list(x)) return(x)
  names(x) <- key_map_to_slot_names(key_map, length(x))
  x
}
...
name_msdial_component <- function(x, component = "peak") {
  if (!is.list(x)) return(x)

  if (!is_named_list(x)) {
    key_map <- switch(
      component,
      peak = CHROM_PEAK_KEY_MAP,
      chromxs = CHROM_XS_KEY_MAP,
      chromx = CHROM_X_KEY_MAP,
      spectrum_peak = SPECTRUM_PEAK_KEY_MAP,
      formula = FORMULA_KEY_MAP,
      adduct = ADDUCT_ION_KEY_MAP,
      match_result = MS_SCAN_MATCH_RESULT_KEY_MAP,
      match_container = MS_SCAN_MATCH_RESULT_CONTAINER_KEY_MAP,
      ion_feature = ION_FEATURE_CHARACTER_KEY_MAP,
      linked_peak = LINKED_PEAK_FEATURE_KEY_MAP,
      peak_shape = CHROMATOGRAM_PEAK_SHAPE_KEY_MAP,
      filter_status = FEATURE_FILTER_STATUS_KEY_MAP,
      NULL
    )
    if (!is.null(key_map)) {
      x <- name_slots_by_key_map(x, key_map)
    }
  }

  if (component == "peak") {
    for (field in c("ChromXsLeft", "ChromXsTop", "ChromXsRight")) {
      if (!is.null(x[[field]])) x[[field]] <- name_msdial_component(x[[field]], "chromxs")
    }
    if (!is.null(x[["Spectrum"]])) {
      x[["Spectrum"]] <- lapply(x[["Spectrum"]], name_msdial_component, component = "spectrum_peak")
    }
    if (!is.null(x[["Formula"]])) x[["Formula"]] <- name_msdial_component(x[["Formula"]], "formula")
    if (!is.null(x[["AdductType"]])) x[["AdductType"]] <- name_msdial_component(x[["AdductType"]], "adduct")
    if (!is.null(x[["TextDbBasedMatchResult"]])) x[["TextDbBasedMatchResult"]] <- name_msdial_component(x[["TextDbBasedMatchResult"]], "match_result")
    if (!is.null(x[["MatchResults"]])) x[["MatchResults"]] <- name_msdial_component(x[["MatchResults"]], "match_container")
    if (!is.null(x[["PeakCharacter"]])) x[["PeakCharacter"]] <- name_msdial_component(x[["PeakCharacter"]], "ion_feature")
    if (!is.null(x[["PeakShape"]])) x[["PeakShape"]] <- name_msdial_component(x[["PeakShape"]], "peak_shape")
    if (!is.null(x[["FeatureFilterStatus"]])) x[["FeatureFilterStatus"]] <- name_msdial_component(x[["FeatureFilterStatus"]], "filter_status")
    if (!is.null(x[["DriftChromFeatures"]])) {
      x[["DriftChromFeatures"]] <- lapply(x[["DriftChromFeatures"]], name_msdial_component, component = "peak")
    }
    return(x)
  }

  if (component == "chromxs") {
    for (field in c("InnerRT", "InnerRI", "InnerDrift", "InnerMz")) {
      if (!is.null(x[[field]])) x[[field]] <- name_msdial_component(x[[field]], "chromx")
    }
    return(x)
  }

  if (component == "match_container") {
    if (!is.null(x[["MatchResults"]])) {
      x[["MatchResults"]] <- lapply(x[["MatchResults"]], name_msdial_component, component = "match_result")
    }
    if (!is.null(x[["MSRawID2MspBasedMatchResult"]])) {
      x[["MSRawID2MspBasedMatchResult"]] <- lapply(x[["MSRawID2MspBasedMatchResult"]], name_msdial_component, component = "match_result")
    }
    if (!is.null(x[["TextDbBasedMatchResults"]])) {
      x[["TextDbBasedMatchResults"]] <- lapply(x[["TextDbBasedMatchResults"]], name_msdial_component, component = "match_result")
    }
    return(x)
  }

  if (component == "ion_feature") {
    for (field in c("AdductType", "AdductTypeByAmalgamationProgram")) {
      if (!is.null(x[[field]])) x[[field]] <- name_msdial_component(x[[field]], "adduct")
    }
    if (!is.null(x[["PeakLinks"]])) {
      x[["PeakLinks"]] <- lapply(x[["PeakLinks"]], name_msdial_component, component = "linked_peak")
    }
    return(x)
  }
...
name_pai_raw <- function(raw_list) {
  lapply(raw_list, name_msdial_component, component = "peak")
}

describe_pai_peak <- function(pai, index = 1L, max.level = 2) {
  peak <- pai[["raw"]][[index]]
  str(peak, max.level = max.level)
  invisible(peak)
}

peak_feature_field_dictionary <- data.frame(
  slot = seq_len(54L) - 1L,
  name = key_map_to_slot_names(CHROM_PEAK_KEY_MAP, 54L),
  stringsAsFactors = FALSE
)

component_field_dictionary <- function(component = c(
  "peak", "chromxs", "chromx", "spectrum_peak", "formula", "adduct",
  "match_result", "match_container", "ion_feature", "linked_peak",
  "peak_shape", "filter_status"
)) {
...
# =============================================================================
# Legacy helper: rename named integer keys to human-readable field names.
# =============================================================================
...
")
  ...
  if (rename_keys) {
    result_r <- name_pai_raw(result_r)
  }
